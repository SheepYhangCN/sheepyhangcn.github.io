...
{% image /assets/images/drzh-tech/p1/1.png width:800px %}
{% image /assets/images/drzh-tech/p1/2.png width:500px %}
{% image /assets/images/drzh-tech/p1/3.png width:200px %}
{% image /assets/images/drzh-tech/p1/4.png width:400px %}
{% endswiper %}
...
## 补丁源码仓库
{% image /assets/images/drzh-tech/p1/5.png width:200px %}
现在我们就可以开始了
首先是比较简单的几个目录
...
## workspace
{% image /assets/images/drzh-tech/p1/6.png width:200px %}
...
{% image /assets/images/drzh-tech/p1/7.png width:600px %}
...
{% image /assets/images/drzh-tech/p2/1.png obj_splashscreen_Create width:600px %}

## 打字机
首先是打字机，我们修改了打字逻辑，增加了字符宽度与字间隙
{% image /assets/images/drzh-tech/p2/2.png obj_writer_Draw width:600px %}
如果有使用过一二章汉化的应该会发现，新的字间隙更大，看着更舒服了
但是又没有 undertale 汉化的字间隙那么大
{% image /assets/images/drzh-tech/p2/3.jpg 上到下分别为：UT，第四章，旧版第二章 width:600px %}

另外，我们还专门修改了第一章的打字机
在默认情况下，打字机都使用 `&（与符号）` 来换行
而 GameMaker 的绘制默认使用 `\n` 换行
从第二章开始，打字机也支援了 `\n` 换行，为了方便我们处理文本，我们将这个功能带回了第一章
{% image /assets/images/drzh-tech/p2/4.png obj_writer_Draw width:600px %}

以及后续章节的打字机还有一个 `` `（反单引号）`` 保留后方特殊字符的功能
打字机有一些用到特殊字符功能，例如前面提到的&换行等
如果我们想在文本里使用这些字符，就可以用这种方式来正常打出
我们同样将这个功能带回了第一章
{% image /assets/images/drzh-tech/p2/5.png obj_writer_Draw width:500px %}
...
{% image /assets/images/drzh-tech/p2/6.png 举例，obj_ch3_closet_Step width:600px %}
非英文时会调用 `scr_84_get_lang_string` 方法
{% image /assets/images/drzh-tech/p2/7.png msgsetloc width:600px %}
从启动游戏时储存的 `lang_map` 中取出文本
{% image /assets/images/drzh-tech/p2/8.png scr_84_get_lang_string width:500px %}
判断非英文使用的是 `is_english` 方法，我们去掉了其判断语言的部分
只保留了检测 `global.lang` 是否存在的部分
{% image /assets/images/drzh-tech/p2/9.png 原版&nbsp;is_english width:600px %}
{% image /assets/images/drzh-tech/p2/10.png 修改后的&nbsp;is_english width:600px %}
...
{% image /assets/images/drzh-tech/p2/11.png scr_kana_check width:400px %}

## 人名翻译切换 & 禁用日文
接着便是三四章开始，增加的游戏内切换人名翻译功能
{% image /assets/images/drzh-tech/p2/12.png obj_initializer2_Create width:600px %}
首先是设置了一个 `global.names` 的变量用于存储这个设置
`0`为不译 `1`为翻译可招揽 `2`为全部翻译
{% image /assets/images/drzh-tech/p2/13.png scr_84_init_localization width:400px %}
把这个设置存储到 `true_config.ini` 中

{% image /assets/images/drzh-tech/p2/14.png scr_84_init_localization width:300px %}
在日文版中，大部分贴图与字体切换都是借助这里的 `sprite_map` 与 `font_map`
所以我们也把本身有日文差分的贴图的人名翻译差分写在了这里
很可惜的是这总归是少数，只有战斗介面的三个人名用了这个系统
{% image /assets/images/drzh-tech/p2/15.png scr_84_init_localization width:300px %}

同时，因为读档介面的语言按钮被人名翻译选项替换了
而且还有很多利用了日文的修改方式
所以我们决定彻底禁用日文
{% image /assets/images/drzh-tech/p2/16.png scr_84_init_localization width:400px %}
无论前面加载到什么语言，都会强制被切换回英文
防止有人之前设置过日文，或是在章节选择选单里设置了日文而出 Bug
（我们没有改动章节选择选单里面的语言选项）

然后就是读档介面的修改
{% image /assets/images/drzh-tech/p2/17.png 读档选单 width:400px %}
把下方这个语言按钮替换为了人名翻译的选项
{% image /assets/images/drzh-tech/p2/18.png DEVICE_MENU_Draw width:400px %}
每次点击会调用 `scr_change_language`
{% image /assets/images/drzh-tech/p2/19.png DEVICE_MENU_Step width:400px %}
所以我们也把这个脚本里面的内容换成了修改人名翻译设置，而非原来的切换语言
{% image /assets/images/drzh-tech/p2/20.png scr_change_language width:400px %}
原本设定语言按钮的位置是放不下四个字的
所以为了放得下，我们手动修改了所有按钮与灵魂的位置
{% image /assets/images/drzh-tech/p2/21.png DEVICE_MENU_Draw width:300px %}
...
{% image /assets/images/drzh-tech/p2/22.png DEVICE_MENU_Draw width:400px %}

## 招揽伙伴选单
自从第二章开始，存档介面新增了查看招揽伙伴的功能
这个在英文版中是被横向压缩了的，因为写不下
{% image /assets/images/drzh-tech/p2/23.png 伙伴介面 width:400px %}
但是中文和日文是写的下的，所以按照日文的值来
{% image /assets/images/drzh-tech/p2/24.png obj_fusionmenu_Create width:400px %}

## 地图内人名翻译
第二章与第四章的光世界地图有两处出现人名
Sans 的店铺招牌与学校里黑板上的 Toriel
{% swiper %}
{% image /assets/images/drzh-tech/p2/25.png Sans&nbsp;的店铺招牌 %}
{% image /assets/images/drzh-tech/p2/26.png 学校黑板上的&nbsp;Toriel %}
{% endswiper %}
这两处因为不是物体，而是地图的一部分，所以一般情况下非常麻烦
好在这两处日文同样需要翻译，而在地图上设置了专门的层
会在日文时显示出来并盖住原有的英文
{% image /assets/images/drzh-tech/p2/27.png room_torielclass width:400px %}
所以我们就借助了这些层，直接将其的贴图替换为人名翻译的贴图
同时也修改了对应的逻辑
{% image /assets/images/drzh-tech/p2/28.png obj_84_lang_helper_Create width:400px %}

## 贴图人名翻译
除此之外还有普通的贴图人名
{% box 第二章&nbsp;Pipis %}
{% image /assets/images/drzh-tech/p2/29.png obj_pipis_enemy_Draw&nbsp;与&nbsp;obj_pipis_egg_bullet_Draw %}
{% endbox %}
{% box 第三章&nbsp;Ramb %}
{% image /assets/images/drzh-tech/p2/30.png obj_room_green_room_Create %}
{% image /assets/images/drzh-tech/p2/31.png room_dw_green_room width:400px %}
{% endbox %}
{% box 第三章&nbsp;T&nbsp;级介绍 %}
{% image /assets/images/drzh-tech/p2/32.png obj_dw_ranking_t_explain_Draw width:600px %}
{% image /assets/images/drzh-tech/p2/33.png spr_dw_ranking_t_explain width:600px %}
{% endbox %}
{% box 第三章&nbsp;Rouxls&nbsp;的飞机拖旗 %}
{% image /assets/images/drzh-tech/p2/34.png obj_rouxls_biplane_flag_Draw width:600px %}
{% image /assets/images/drzh-tech/p2/35.png spr_rouxls_biplane_flag %}
{% endbox %}
{% box 第三章&nbsp;Tenna&nbsp;初见视频 %}
{% image /assets/images/drzh-tech/p2/36.png obj_ch3_couch_video_Create %}
{% endbox %}
人名翻译切换相关的改动大概就是这些
...
是按照当前位置，逐字亮起的
所以需要按照汉字的位置重写
{% image /assets/images/drzh-tech/p2/37.png obj_welcometothecity_backinglights_Draw，右侧绿色的都是原本的代码 width:300px %}
{% endbox %}
{% box 第三章的音游歌词 %}
改为了按照日文的方式来显示字间隙
{% image /assets/images/drzh-tech/p2/38.png scr_rhythmgame_lyrics %}
{% image /assets/images/drzh-tech/p2/39.png scr_rhythmgame_lyrics %}
{% endbox %}
{% box 第三章&nbsp;Tenna&nbsp;的长难句 %}
因为中文字少，所以持续时间也短
所以就加长了这个循环，加长了重复的时长
{% image /assets/images/drzh-tech/p2/40.png scr_rhythmgame_lyrics width:400px %}
{% endbox %}
{% box 第三章的饮水机 %}
具体的介绍可以看[这期专栏](https://www.bilibili.com/read/cv42255231)，里面有提到这里的改动
{% image /assets/images/drzh-tech/p2/41.png scr_rhythmgame_lyrics width:400px %}
{% endbox %}
{% box 第三章&nbsp;Rouxls&nbsp;战的彩蛋 %}
这个文本在日文是贴图，英文是文本
为了方便与美观，中文也用了贴图的形式
{% image /assets/images/drzh-tech/p2/42.png obj_rouxls_annyoing_dog_controller_Draw width:400px %}
{% image /assets/images/drzh-tech/p2/43.png spr_txt_ja_annoyingdog width:400px %}
{% endbox %}
{% box 第四章的对话 %}
这里文本有一个特殊字符，而这个字符是一个贴图
Toby 的设计是，读取当前的打字机进度，到了就绘制这个贴图出来
中文译文里没有这种字符，所以就整个代码注释掉
{% image /assets/images/drzh-tech/p2/44.png width:400px %}
{% image /assets/images/drzh-tech/p2/45.png obj_dw_church_intro_guei_Draw width:400px %}
{% endbox %}
{% box 第四章的&nbsp;TAKING&nbsp;TOO&nbsp;LONG %}
这里改动主要是 TAKING TOO LONG 文本的时长与顺序等
{% image /assets/images/drzh-tech/p2/46.png obj_takingtoolong_Draw width:300px %}
{% image /assets/images/drzh-tech/p2/47.png obj_takingtoolong_Draw width:400px %}
{% endbox %}
...
{% image /assets/images/drzh-tech/p2/48.png width:400px %}
{% endbox %}
...
完成后再将构建好的文本复制到输出目录
{% image /assets/images/drzh-tech/p3/1.png width:400px %}
Loader 比较简单，只是提供了日志分级与加载 data 的衔接
{% image /assets/images/drzh-tech/p3/2.png width:200px %}
...
{% image /assets/images/drzh-tech/p3/3.png width:400px %}
在 `ExportTexts` 方法导出文本，其调用了 `DecompileCodes` 方法，批量反编译所有代码
{% image /assets/images/drzh-tech/p3/4.png width:400px %}
{% image /assets/images/drzh-tech/p3/5.png width:400px %}
接着用正则匹配出文本，还有一些正则替换
包括 #& 都替换为真正的换行，去掉标点前的`^1`，去掉文本首尾的控制字符
{% image /assets/images/drzh-tech/p3/6.png width:400px %}
{% image /assets/images/drzh-tech/p3/7.png width:400px %}
...
{% image /assets/images/drzh-tech/p3/8.png width:400px %}
最后我们还是手动给所有不需要 `^1` 的文本前面打 `@` 字符来标记才解决了这个问题
{% image /assets/images/drzh-tech/p3/9.png width:400px %}
{% endbox %}

#### 字体 & 代码 & 贴图
{% box ExportFont&nbsp;导出字体 %}
这个导出来主要是用于后续导入时与中文字体合并
{% image /assets/images/drzh-tech/p3/10.png width:600px %}
{% endbox %}
{% box ExportCodes&nbsp;导出代码 %}
导出所有代码，用于修改
当然这个用处不大，因为游戏更新，后续一般都是直接用 UTMT 改了复制出来
而不是全部导出来再挑出需要用的
{% image /assets/images/drzh-tech/p3/11.png width:600px %}
{% endbox %}
{% box ExportSprites&nbsp;导出贴图 %}
导出所有贴图
{% image /assets/images/drzh-tech/p3/12.png width:600px %}
{% endbox %}

### Importer 导入
最重要的 `Importer`，用于导入所有汉化修改后的文本资源代码
透过多线程运行，分别生成字体，处理文本，导入贴图与代码
{% image /assets/images/drzh-tech/p3/13.png width:400px %}

#### ImportFont 字体
首先是 `ImportFont`，这会用到一个字库
这个字库是自动构建的，包含这些东西：
`code` 目录下的代码、`text_src` 下的中文译文、`global` 目录下的中文人名
{% image /assets/images/drzh-tech/p3/14.png width:400px %}
然后将其以 UTF-16 编码输出到 `dump` 目录下的 `dict.txt` 中
{% image /assets/images/drzh-tech/p3/15.png width:400px %}
（别问我为什么是 UTF-16，老资历 bmfont 导致的）
接下来便会读取 bmfc 基础配置，并遍历字体对应的所有图片
然后按照这些图片的文件名，分割出`字符 ID`、`X 轴偏移`和 `X 轴 Advance`
{% image /assets/images/drzh-tech/p3/16.png width:400px %}
把这些参数追加到基础配置中，生成完整的配置
{% image /assets/images/drzh-tech/p3/17.png width:400px %}
接着便是使用 bmfont 生成字体大图与 `fnt`，完成后调用 `ImportFontData`
{% image /assets/images/drzh-tech/p3/18.png width:400px %}

从输出的 `fnt` 中读取字形参数，并设置到新建的字形资源中
{% image /assets/images/drzh-tech/p3/19.png width:400px %}
同时还有中英混排时出现字形偏移的修复，以及字形切尾的修复
{% image /assets/images/drzh-tech/p3/20.png width:500px %}
将这些修复应用到字形资源中
{% image /assets/images/drzh-tech/p3/21.png width:600px %}
完成后便创建一个新的纹理页，大小与字体纹理相同，将字体纹理放进纹理页中
然后再从 data 中找出同名字体，设置其的参数与对应的纹理
以及把前面创建的字形资源赋值给这个字体，将其覆盖
{% image /assets/images/drzh-tech/p3/22.png width:300px %}

#### ImportTexts 文本
接着是 `ImportTexts`
按照 `text_src` 下的 `raw.json` 遍历，一个个匹配
{% image /assets/images/drzh-tech/p3/23.png width:300px %}
再透过 `ReplaceItem` 方法来替换人名译文
人名译文替换时各种命名介面是例外，需要防止替换到命名彩蛋等
{% image /assets/images/drzh-tech/p3/24.png width:300px %}
...
{% image /assets/images/drzh-tech/p3/25.png width:300px %}

#### ImportCodes 代码
`ImportCodes`，导入代码
这个比较简单，直接使用 `CodeImportGroup` 类即可
{% image /assets/images/drzh-tech/p3/26.png width:400px %}
...
{% image /assets/images/drzh-tech/p3/27.png width:400px %}
会透过 `GetSprite` 方法获取贴图
如果匹配不到贴图那就尝试匹配去掉 `_zhname` 的原贴图
{% image /assets/images/drzh-tech/p3/28.png width:400px %}
若匹配到原贴图后新建一个贴图，并照抄原有贴图的参数，将其添加到 data 后再返回
{% image /assets/images/drzh-tech/p3/29.png width:400px %}
拿到贴图之后设置其的纹理并调整偏移量
如果纹理大小不一致，则会重新计算原点与边界的值
{% image /assets/images/drzh-tech/p3/30.png width:400px %}

#### 导入结果
全部完成导入后就把 data 输出到 result 目录，这样就完成了一次构建
{% image /assets/images/drzh-tech/p3/31.png width:600px %}
...
{% image /assets/images/drzh-tech/p4/1.jpg 因为我会（x width:200px %}
...
{% image /assets/images/drzh-tech/p4/2.png width:600px %}

接着便是主介面，包含中间的文字与按钮
`CenterContainer` 的功能是将节点居中
{% image /assets/images/drzh-tech/p4/3.png width:600px %}

`BottomContainer` 是类型为 `HBoxContainer` 的容器，用于把节点横向排列
用于左下角的安装器版本号与右下角的安装器更新按钮
{% image /assets/images/drzh-tech/p4/4.png width:600px %}
这里的 `Left` 同理，是类型为 `VBoxContainer` 的容器，用于把节点纵向排列
{% image /assets/images/drzh-tech/p4/5.png width:600px %}

然后便是单独的：
左上角资讯按钮，右上角语言选项，用于 macOS 的窗口标题栏
{% image /assets/images/drzh-tech/p4/6.png width:600px %}
还有一个弹出选取文件用的 `FileDialog`
{% image /assets/images/drzh-tech/p4/7.png width:400px %}
...
{% image /assets/images/drzh-tech/p4/8.png width:600px %}
...
{% swiper %}
{% image /assets/images/drzh-tech/p4/9.png 变量定义 width:100px %}
{% image /assets/images/drzh-tech/p4/10.png 编辑器 width:100px %}
{% endswiper %}

安装器初始化时会检查是否是用于旧设备的 Outdated 版本，如果是则弹出提示
这是通过检查引擎版本是否小于等于 4.4 来实现的
{% image /assets/images/drzh-tech/p4/11.png %}
{% image /assets/images/drzh-tech/p4/12.png %}
...
{% image /assets/images/drzh-tech/p4/13.png width:400px %}
然后还有检查是否有 `readme` 文件，有就透过窗口显示出来
{% image /assets/images/drzh-tech/p4/14.png width:400px %}

接着便是更新 Contributor 列表，检查补丁与安装器更新
{% swiper %}
{% image /assets/images/drzh-tech/p4/15.png 更新&nbsp;Contributor&nbsp;列表 %}
{% image /assets/images/drzh-tech/p4/16.png 检查补丁更新 %}
{% image /assets/images/drzh-tech/p4/17.png 检查安装器更新 %}
{% endswiper %}
这些操作都是使用 `HttpClient`，透过 Github API 完成的
{% image /assets/images/drzh-tech/p4/18.png %}
{% image /assets/images/drzh-tech/p4/19.png %}
{% image /assets/images/drzh-tech/p4/20.png %}
补丁与安装器的更新讯息，包括版本号与链接，都会各自存储到 `patchreleases` 与 `patcherreleases` 中
{% image /assets/images/drzh-tech/p4/21.png %}
{% image /assets/images/drzh-tech/p4/22.png %}

如果之前没有填写过路径，那么就会透过 `FindGamePath` 方法尝试寻找游戏路径
{% image /assets/images/drzh-tech/p4/23.png %}
其会先检查这些默认路径
```
...
{% image /assets/images/drzh-tech/p4/25.png width:600px %}
{% image /assets/images/drzh-tech/p4/24.png width:600px %}
...
{% image /assets/images/drzh-tech/p4/26.png width:400px %}

#### 安装逻辑
{% image /assets/images/drzh-tech/p4/27.png width:400px %}
点击安装按钮，会先使用 `PathTrim` 方法来进行路径处理
包括处理正反斜杠，替换类 Unix 的 `~` 家目录，补全 macOS 包内路径
{% image /assets/images/drzh-tech/p4/28.png width:400px %}
...
{% image /assets/images/drzh-tech/p4/29.png width:400px %}
譬如检查当前系统中是否有能直接用的，以及 sha256 散列值检查
{% image /assets/images/drzh-tech/p4/30.png %}
{% image /assets/images/drzh-tech/p4/31.png width:400px %}
要用到 sha256 散列值检查是因为之前遇到过有人电脑中了病毒
然后解压安装器时自动感染了，然后顺带还损坏了程序

检查完成后便开始安装，使用 `Process` 类新建进程运行 7z
{% image /assets/images/drzh-tech/p4/32.png 7z.exe&nbsp;<补丁路径>&nbsp;-o&nbsp;<安装器目录下&nbsp;ExtractTemp>&nbsp;-aoa&nbsp;-y width:400px %}
用 7z 解压后调用 `MoveAfterExtracted` 方法将文件全部移动到 DELTARUNE 目录下
{% image /assets/images/drzh-tech/p4/33.png %}
移动前会检查是否已经存在文件，如果存在那么就把原有的文件移动到 `backup` 目录下，再移动
{% image /assets/images/drzh-tech/p4/34.png width:400px %}
{% image /assets/images/drzh-tech/p4/35.png width:400px %}
然后再备份 data，创建多进程为每个 data 运行 xdelta3
{% image /assets/images/drzh-tech/p4/36.png width:400px %}
{% image /assets/images/drzh-tech/p4/37.png width:400px %}
直到所有 data 完成后才会触发结束
{% image /assets/images/drzh-tech/p4/38.png %}
若 30 秒后还没完成就杀死进程并触发 `TAKING TOO LONG` 错误
{% image /assets/images/drzh-tech/p4/39.png width:400px %}

安装结束进入 `Ending` 方法，清理所有 xdelta 文件
{% image /assets/images/drzh-tech/p4/40.png width:400px %}
计算安装耗时，根据日志内容触发对应的错误到 `PatchResultHandler` 方法
{% image /assets/images/drzh-tech/p4/41.png width:400px %}
保存 log，若失败则透过 `RestoreData` 方法撤销更改
{% image /assets/images/drzh-tech/p4/42.png 保存&nbsp;log width:400px %}
{% image /assets/images/drzh-tech/p4/43.png PatchResultHandler width:400px %}
最后显示结果弹窗，安装流程结束

卸载补丁本质上就是将 `backup` 目录里的文件覆盖到游戏文件中
按下按钮后便会检查备份存在，然后触发 `RestoreData` 方法，遍历 `backup` 目录还原
{% image /assets/images/drzh-tech/p4/44.png width:400px %}
{% image /assets/images/drzh-tech/p4/45.png width:400px %}

#### 补丁更新逻辑
点击更新补丁时，会先删除现有的补丁
{% image /assets/images/drzh-tech/p4/46.png width:600px %}
然后从 `patchreleases` 的 `assets` 获取 `name`，检查是否 demo 与操作系统
接着获取它的 `browser_download_url`
{% image /assets/images/drzh-tech/p4/47.png width:600px %}
使用 `HttpClient`，按照 4096 字节分块异步下载
{% image /assets/images/drzh-tech/p4/48.png width:600px %}
下载中的文件会在文件名前面加上 `_downloadingtemp_`
这是为了防止在某些异常情况下没下完的文件被安装器识别为正常补丁
{% image /assets/images/drzh-tech/p4/49.png %}
{% image /assets/images/drzh-tech/p4/50.png %}
完成后等待五秒重载场景
{% image /assets/images/drzh-tech/p4/51.png %}