...
重量级出现了
{% image /resources/images/uty/uty/dialogue_battle.png %}
是的 每一处打字机都是一个物体
然后我又搜索了dialogue
更加印证了我的想法
...
首先是开头的故事intro
我找到了文本 翻译好了 一切正常
{% image /resources/images/uty/uty/intro.png %}
纸鸢提供了那张寻人启事的图
...
所以我的决定是 直接修改draw函数
byd谁闲着给你一点一点调位置
{% image /resources/images/uty/uty/z_enter_original.png %}
↓改完之后↓
{% image /resources/images/uty/uty/z_enter_improved.png %}

然后就是menu和instruction
翻译了menu后 我发现找不到instruction的文本在哪
此时我还调侃说不会是个贴图吧
然后我还就真找到了
{% image /resources/images/uty/uty/instruction.png %}
不是 你没病吧 你和RickyG学的是吧
对此 我的选择是
直接给你字删了然后手动代码绘制一份
{% image /resources/images/uty/uty/menu.png %}

## Overworld
然后就是ow的测试
首先是测试了有无空格的差别
{% image /resources/images/uty/uty/no_space.png %}
{% image /resources/images/uty/uty/with_space.png %}
显而易见了 所以就要求必须打空格
...
{% image /resources/images/uty/uty/color.png %}
↑比如这里一共是三个打字机↑
而且这个东西的打字速度还是一样的
...
前面说过 战斗打字机得改
然后这里有一张打字机没改时的珍贵录像（
{% image /resources/images/uty/uty/battle.png %}
